...
const DRUGS = [
  ...
  { key: "atropine", name: "Atropine", cost: 600, avail: "5/12 hrs", legality: "Legal", si: 1,
    ...
    card: "+1 TN to all Active Skill tests (+1 more for melee and close-range firearms); +2 TN to Knowledge, Technical, B/R, Language and Magic skills; hallucinations, fever. Damage repeats every 15 minutes until neutralised or filtered out (Shadowtech p.96).",
  ...
  { key: "hyper", name: "Hyper", cost: 180, avail: "4/24 hrs", legality: "4-M1", si: 0.9,
    ...
    card: "Vertigo: +1 to all TNs; +4 to concentration tasks (spellcasting included); any damage taken while it lasts adds half its boxes (round up) as Stun — the GM applies these (Shadowtech p.98).",
  ...
  { key: "kamikaze", name: "Kamikaze", cost: 50, avail: "5/4 days", legality: "3-M1", si: 5,
    ...
    card: "Also negates the first 4 boxes of damage taken after it's administered (any mix of Physical and Mental) — the GM applies it. Every 4 uses cost a box off both monitors' maximum; after (Body ÷ 2) uses, cyberware and bioware stop working (Shadowtech p.99).",
